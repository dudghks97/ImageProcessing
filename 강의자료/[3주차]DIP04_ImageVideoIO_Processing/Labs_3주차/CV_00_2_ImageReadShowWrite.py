...
# ----------------------------------------------------------------------------------------------------------------------
# 단계 1: 영상 파일의 영상 읽기. 다음 함수를 통해 지정된 영상을 읽어내어 영상 데이터를 numpy.ndarray 객체로 반환한다.
# cv.imread(파일, flags)에서; flags : -1=있는 그대로 읽기. 0=모노(1채널)로 읽기, 1=칼라(3채널)로 읽기(default)
# ----------------------------------------------------------------------------------------------------------------------
...
# 저장한 파일을 다시 읽어낸다.
img = cv.imread(file_mono)   # -1 : 있는 그대로
...
# 0. fname으로 지정된 파일 출력
# 1. 가로 및 세로 정보를 a, b 에 넣기

# 2. 영상의 면적 출력

# 3. 파일의 컬러 여부 검사 후 결과 출력

# 4. 해당 영상 화면 출력 파일 이름 : 타이틀 바에 출력 되어야함

# 5. 영상 quality 지정하여 현재의 폴더에 jpg영상으로 저장 파일 이름 : tmp.jpg quality : 사용자 입력으로
